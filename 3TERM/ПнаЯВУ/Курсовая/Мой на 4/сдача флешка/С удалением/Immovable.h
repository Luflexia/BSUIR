...
class Immovable {
    class ContactDetails {
        ...
        string email;
    public:
        ContactDetails(const string& phonePrimary = "", const string& email = "") {
...
public:
    Immovable(unsigned int id, const string& phone = "", const string& email = "", float cost = -1,
        double square = -1, const string& address = "", bool actuality = true) {
        this->id = id;
    ...
    //  square
    double setSquare(double value) {
        return square = value;
    }

    string printSquare() {
        stringstream stream;
        stream << std::fixed << std::setprecision(1) << std::showpoint << square;
        return stream.str() + " м^2";
    }
    //string printSquare() {
    //    return to_string(square) + " м^2";
    //}
    ...
    string printCost() {
        stringstream stream;
        stream << fixed << setprecision(2) << std::showpoint << cost;
        return stream.str() + "$";
    }
       //string printCost() {
