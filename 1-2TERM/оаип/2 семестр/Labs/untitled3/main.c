...
//------------------------------------------------------------------------------------------------------------------------------------
void del(FILE* stream, char *name) // сдвигаем последний элемент на место удаляемого и уменьшаем размер файла
{
    info info;
    fpos_t l, k, m;

    while (fread(&info, sizeof(info), 1, stream) && strcmp(info.surname, name)) {

    }

                                                                              //  1. находим удаляемый элемент

    if (strcmp(info.surname, name) == 0) {

        l = ftell(stream); // указатель после удаляемой структуры

                                                                              // 2. находим последний элемент

        fseek(stream, 0, SEEK_END);                          // перешли на конец файла

        k = ftell(stream);                                               // запомнили место конца

        fseek(stream, 0, SEEK_SET);

        fseek(stream, k-sizeof(info), 0);
        fread(&info, sizeof(info), 1, stream);    // читаем последний элемент


        fseek(stream, l - (sizeof(info)), SEEK_SET);         // возвращаемся к началу удаляемого
        fwrite(&info, sizeof(info), 1, stream);           // переписываем удаляемый
    }
    puts("After overwriting");
    output(stream);
    _chsize(fileno(stream),k-sizeof(info));
    puts("After downsizing");
    output(stream);
};
//E:\Downloads\Labs\untitled3\cmake-build-debug\untitled3 -1
