...
void out(node* r1, char r)//функция вывода дерева
{
    ...
    {
        cout « r « ":";//выводим символ обозначающий к какой части дерева относится переменная хранящаяся в ветви
        cout « r1->data « " ";//вывод саму переменную
    }
...
int main() //думаю тут уже комент не нужен
{
    ...
    cin » n;//ввод размера очереди\дерева
    for (int i = 0; i < n; i++)
    {
        cin » mp;//считываем переменную
    ...
    cout « "h:" « root->data«" ";//выводим корень дерева с символом h(head)
