...
void main() {
	old9 = getvect(0x09); // сохраняем указатель на старый обработ-чик
...
void indicator(int mask) {
// перед отправлением маски отправляем код 0xED - код команды управ-ления светодиодами клавиатуры
	if (mask != 0xED) indicator(0xED);
	int i = 0;
// ждем подтверждения, что внутренняя очередь команд процессора кла-виатуры пуста (считываем слово состояния)
