...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle('Управление базой данных Railway (Berkeley DB)')
        ...
        # Бейдж с типом backend
        self.db_badge = QLabel('Backend: Berkeley DB (JE)')
        self.db_badge.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.db_badge.setStyleSheet('color: #0a6; font-weight: bold; font-size: 14px;')
        left_layout.addWidget(self.db_badge)
        ...
        self.query_edit.setPlaceholderText('Введите SQL-запрос (Berkeley DB). Пример: SELECT * FROM "Сотрудник" WHERE Должность contains \'Машинист\'')
    ...
    def init_menu_and_shortcuts(self):
        # Меню-бар с мнемониками (&File, &Databases, &Operations)
        menubar = self.menuBar()
        self.file_menu = menubar.addMenu("&File")
        self.databases_menu = menubar.addMenu("&Databases")
        ...
        # --- Databases --- (все операции таблиц/баз)
        act_refresh_tables = QAction('Обновить список', self)
        act_refresh_tables.triggered.connect(self.update_tables_list)
        self.databases_menu.addAction(act_refresh_tables)

        act_select_table = QAction('Выбрать активную базу...', self)
        act_select_table.setShortcut(QKeySequence('Alt+D'))
        act_select_table.setShortcutContext(Qt.ShortcutContext.ApplicationShortcut)
        act_select_table.triggered.connect(self.select_active_table)
        self.addAction(act_select_table)
        self.databases_menu.addAction(act_select_table)

        act_view_table = QAction('Просмотреть активную базу', self)
        ...
        self.databases_menu.addAction(act_view_table)

        self.databases_menu.addSeparator()
        ...
        self.databases_menu.addAction(act_add_row)
        ...
        self.databases_menu.addAction(act_delete_row)
        ...
        self.databases_menu.addAction(act_update_row)

        self.databases_menu.addSeparator()

        act_create_table = QAction('Создать базу', self)
        act_create_table.triggered.connect(self.create_table)
        self.databases_menu.addAction(act_create_table)

        act_delete_table = QAction('Удалить базу', self)
        act_delete_table.triggered.connect(self.delete_table)
        self.databases_menu.addAction(act_delete_table)
    ...
    def update_tables_list(self):
        self.worker = Worker(requests.get, f"{self.api_url}/tables", auth=self.get_auth())
        self.worker.finished.connect(self.on_update_tables_list_finished)
        self.worker.error.connect(self.on_request_error)
        self.worker.start()

    def on_update_tables_list_finished(self, response):
        try:
    ...
    def delete_row(self):
        ...
        key_col = headers.index('_key') if '_key' in headers else 0
        row_key_item = self.table_widget.item(row_index, key_col)
        
        if not row_key_item:
             QMessageBox.warning(self, 'Предупреждение', 'Не удалось определить ключ записи')
             return
        
        row_id = row_key_item.text()

        reply = QMessageBox.question(
            self, 'Подтверждение',
            f'Вы уверены, что хотите удалить строку с ключом={row_id}?',
    ...
    def edit_cell(self, row, column):
        ...
        if header.lower() == '_key':
            QMessageBox.information(self, 'Информация', 'Ключ (_key) не может быть отредактирован напрямую')
            return
            
        headers = [self.table_widget.horizontalHeaderItem(i).text() for i in range(self.table_widget.columnCount())]
        key_col = headers.index('_key') if '_key' in headers else 0
        row_key_item = self.table_widget.item(row, key_col)
        ...
        if ok and new_value != current_value:
            full_row_data = {}
            for j in range(self.table_widget.columnCount()):
                 h = self.table_widget.horizontalHeaderItem(j).text()
                 val = self.table_widget.item(row, j).text()
                 full_row_data[h] = val
            
            full_row_data[header] = new_value
            
            try:
                response = requests.put(
                    f"{self.api_url}/tables/{self.current_table}/rows/{row_id}",
                    auth=self.get_auth(),
                    json={"data": full_row_data}
                )
                response.raise_for_status()
    ...
    def update_row(self):
        ...
        self.worker.finished.connect(lambda response, r=row_index, c_name=col_name, n_val=new_value: self.on_update_row_finished(response, r, c_name, n_val))
        self.worker.error.connect(self.on_request_error)
        self.worker.start()

    def on_update_row_finished(self, response, row_index, col_name, new_value):
        try:
            response.raise_for_status()
            # Обновим в таблице
            for j in range(self.table_widget.columnCount()):
    ...
    def execute_query(self):
        ...
        self.worker.finished.connect(self.on_execute_query_finished)
        self.worker.error.connect(self.on_request_error)
        self.worker.start()

    def on_execute_query_finished(self, response):
        try:
            response.raise_for_status()
            result = response.json()

            if isinstance(result, list):
                # Если вернулся список (данные)
                self.table_widget.setRowCount(len(result))
                if not result:
                    self.table_widget.setColumnCount(0)
                    QMessageBox.information(self, 'Результат', 'Запрос не вернул данных')
                    return
                
                # Собираем все возможные колонки из данных
                columns = set()
                for row in result:
                    columns.update(row.keys())
                
                # Сортируем, чтобы _key был первым, а остальные по алфавиту
                sorted_columns = sorted(list(columns))
                if '_key' in sorted_columns:
                    sorted_columns.remove('_key')
                    sorted_columns.insert(0, '_key')
                
                self.table_widget.setColumnCount(len(sorted_columns))
                self.table_widget.setHorizontalHeaderLabels(sorted_columns)
                ...
                # Если вернулся словарь (например, сообщение об успехе или старый формат)
                if "columns" in result and "data" in result:
                 ...
                 QMessageBox.warning(self, 'Ошибка', 'Неизвестный формат ответа от сервера')
    ...
    def export_to_excel(self):
        ...
        self.worker.finished.connect(lambda response, fp=file_path: self.on_export_to_excel_finished(response, fp))
        self.worker.error.connect(self.on_request_error)
        self.worker.start()

    def on_export_to_excel_finished(self, response, file_path):
        try:
            response.raise_for_status()
            data = response.json()
    ...
    def backup_database(self):
        self.worker = Worker(
            requests.post,
            f"{self.api_url}/database/backup",
            auth=self.get_auth()
        )
        self.worker.finished.connect(self.on_backup_database_finished)
        self.worker.error.connect(self.on_request_error)
        self.worker.start()

    def on_backup_database_finished(self, response):
        try:
            response.raise_for_status()
    ...
    def restore_database(self):
        file_path, _ = QFileDialog.getOpenFileName(self, 'Выбрать файл резервной копии', '', 'Zip Files (*.zip);;All Files (*)')
        if not file_path:
            return

        self.worker = Worker(
            requests.post,
            f"{self.api_url}/database/restore",
            auth=self.get_auth(),
            json={"backup_file": file_path}
        )
        self.worker.finished.connect(self.on_restore_database_finished)
        self.worker.error.connect(self.on_request_error)
        self.worker.start()

    def on_restore_database_finished(self, response):
        try:
            response.raise_for_status()
            QMessageBox.information(self, 'Успех', response.json().get("message"))
            self.update_tables_list()
    ...
    def create_table(self):
        ...
        self.worker.finished.connect(lambda response, tn=table_name: self.on_create_table_finished(response, tn))
        self.worker.error.connect(self.on_request_error)
        self.worker.start()

    def on_create_table_finished(self, response, table_name):
        try:
            response.raise_for_status()
    ...
    def delete_table(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self.worker = Worker(
                requests.delete,
                f"{self.api_url}/tables/{self.current_table}/delete",
                auth=self.get_auth()
            )
            self.worker.finished.connect(lambda response, tn=self.current_table: self.on_delete_table_finished(response, tn))
            self.worker.error.connect(self.on_request_error)
            self.worker.start()

    def on_delete_table_finished(self, response, table_name):
        try:
            response.raise_for_status()
