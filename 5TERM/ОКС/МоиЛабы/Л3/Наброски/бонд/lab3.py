...
class COMPort:
    ...
    def _read_loop(self, output_textbox, status_label):
        ...
        while self._init_flag and not self._stop_thread:
            try:
                ...
                messagebox.showerror("Ошибка COM порта", f"Ошибка чтения с COM порта {self._port_name}: {str(e)}")
                break


    def update_message_count(self, status_label, packets=None, modified_bytes=None):
        if packets is None:
            packets = []
        ...
        static_info = (
            f"Количество переданных порций: {self.messages_received}\n"
        )
        ...
        for i, packet in enumerate(packets):
            ...
            crc = packet[-1]
            packet_info += f"Пакет {i+1}: {flag}{destination_address}{source_address}{data} {crc}\n"

        status_text = "\nСтруктура пакетов:\n" + packet_info + static_info
    ...
    def write_to_port(self, message: str, status_label):
        ...
        try:
            for packet in packets:
                ...
                print(stuffed_packet)
                packed_packets.append(stuffed_packet)
                modified_bytes.extend(modified)  
            self._portID.write(b'\n')
            self.messages_received += 1  
            return packed_packets, modified_bytes  
        except serial.SerialException as e:
...
class SerialApp:
    ...
    def send_message(self, event):
        ...
        if message and self.tx_port and self.tx_port._init_flag:
            packets, modified_bytes = self.tx_port.write_to_port(message, self.status_label)
            self.input_textbox.delete("1.0", tk.END)
            self.tx_port.update_message_count(self.status_label, packets, modified_bytes)
            return "break"
        return None
