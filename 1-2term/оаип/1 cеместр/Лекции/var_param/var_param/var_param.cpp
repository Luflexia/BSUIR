...
//	p = &k;                  // p указатель на элемент k стека параметров
//	p = ((int *)p) + 1;      // указатель приводится к типу указателя на int 
//	// (т.е. добавляется шаг) и меняется на адрес 
//	// следующего эл-та стека
//	while (k--)              // пока в стеке есть (несчитанные) параметры
//	{
//		s += *((int *)p);
//		p = ((int *)p) + 1;   // переход к новому параметру стека
//	}
//	printf("\n 1 сумма  элементов = %d\n", s);
//}
//
...
//  puts("\n\n функция  - fun 2 -");
//  while(*(char **)p)       // цикл пока в стеке не nullptr  (int **)p
//  { printf("\n%s",*(char **)p++);  // как и в функции выше
//    //p++;	             // переход к новому элементу (адресу) в стеке
...
//
//void var_fun(int k, int l, data tp, ...)    // k-число групп
//{ 
...
//				p = ((double*)p) + 1;                    // p перемещается на следующий double эл-т стека
//		    }
//		    break;
//		    case 'i':   for (j = 0; j<i; j++)
//		    {
//				**((int**)sum + 0) += *(int *)p;         // накопление суммы элементов int
//				p = ((int*)p) + 1;                       // p перемещается на следующий double эл-т стека
//		    }
//		}
//	}
...
//  void **ii;            
//  void *jj;
//  jj = (int *)&n;   // считывается число указателей на массивы (т.е. число массивов) (2)
//  printf("%3d", *(int*)jj);
//  jj=(int*)jj + 1;  k = *((int*)jj);       // число - размерность первого массива (3)
//  while (1)
//  {	  printf("\n");
//	  printf("%3d   ", k);
